# Contributing to SF Log Analyzer
...
4. Test against a real Salesforce debug log if possible
...
# Right-click a .log file → SF Log Analyzer: Load in Analyzer
```

Changes to `media/main.js` and `media/styles.css` take effect after closing and reopening the panel — no recompile needed. Changes to `src/logPanel.ts` or `src/extension.ts` require `npm run compile` and a panel reload.

## No guarantees

This project is maintained in spare time. Issues and PRs may take time to review. If you need a fix urgently, the codebase is intentionally readable — feel free to fork and patch.
